...
public class Inventory {
	...
	int n = knapsack.getCapacity() / 5, m = 5, 
	    w = item.getSizeJ(), h = item.getSizeI();
	
	if (item instanceof CountableItem) {
	    CountableItem сi = (CountableItem) item;
	    int sum = сi.getAmount();
	    for (Item e : items)
	...
	for (int i = 0; i < n - h + 1; i++)
		...
		boolean ok = true;
		for (Item e : items) {
		    ok &= (!Utils.boundingBox(i, j, i + h - 1, j + w - 1,
			    e.getI(), e.getJ(), e.getI() + e.getSizeI() - 1,
			    e.getJ() + e.getSizeJ() - 1));
		}
		if (ok) {
		    return true;

...
public class StateView extends Sprite {
	...
	dropOut = new Sprite(getWidth(), 0, "gui/dark_bg") {
		...
		dropOut.setPosition(StateView.this.getWidth() + 1,
			StateView.this.getHeight() - getHeight());
	    }
	};
	dropOut.setSize(150, 40);

	valText = new Text(5, 5, "хз");
	valText.setColor(.8f, .8f, .8f);
	...
	if (Utils.isSelected(this, mx, my))
	    dropOut.show();
	if (!Utils.isSelected(this, mx, my)
		&& !Utils.isSelected(dropOut, mx, my))
	    dropOut.hide();
    }
}
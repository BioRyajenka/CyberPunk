...
public class Utils {
     ...
     */
    public static boolean boundingBox(int ax1, int ay1, int ax2, int ay2,
	...
	return (Utils.inBounds(mx, s.getGlobalX(),
		s.getGlobalX() + s.getWidth()) && Utils.inBounds(my,
		s.getGlobalY(), s.getGlobalY() + s.getHeight()));
    }

    public static String floatToString(float f, int k) {
	String fract = String.valueOf(f - (int) f);
	return String.valueOf((int) f) + "."
		+ fract.substring(2, Math.min(fract.length(), k + 2));
    }
	...
	while (uk1 != uk2) {
		...
		for (int dj = -1; dj <= 1; dj++) {
			...
			continue;
		    int ni = i + di, nj = j + dj;
		    if (validator.apply(ni, nj)
			    && (d[ni][nj] == -1 || d[i][j] + 1 < d[ni][nj])) {
			queue[uk2][0] = ni;
			queue[uk2++][1] = nj;
			d[ni][nj] = d[i][j] + 1;
		    }
	...
	public BresenhamLineIterator getIterator() {
	    return new BresenhamLineIterator(start.first, start.second,

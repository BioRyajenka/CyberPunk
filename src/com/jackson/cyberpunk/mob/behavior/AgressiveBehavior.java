...
public class AgressiveBehavior extends Behavior {
	...
	public void doLogic(NPC handler) {
		...
		if (!isChasingPlayer()) {
			...
			handler.setBehavior(new WanderBehavior());
			return;
		}
		...
		if (!handler.isSeeMob(pl) || (handler.getWeapon().isMelee() && !handler
				.isMobNear(pl))) {
	...
	private void wieldCoolestWeapon(NPC handler) {
		...
		for (Item i : inv.getItems()) {
			if (i instanceof Weapon) {
				Weapon w = (Weapon) i;
				if (getWeaponTypeCoolness(w.getWeaponType()) > getWeaponTypeCoolness(resw
						.getWeaponType()) && hasAmmo(handler, w.getAmmoType())) {
					resw = w;
				}
			}
		}
